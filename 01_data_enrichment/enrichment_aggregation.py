...
class ExperimentConfig:
    ...
    PROMPT_STRATEGIES = ["zero_shot_enrichment_real_data_aggregation"]
    NROWS = [10]
...
def build_prompt(profile: Dict[str, Any], prompt_template: str) -> str:
    """Build prompt by replacing {{key}} placeholders with profile values."""
    result = prompt_template
    
    # Replace all placeholders like {{key}} with values from profile
    placeholders = [
        "valueuom", "ref_range_lower", "ref_range_upper",
        "valuenum_min", "valuenum_max", "valuenum_mean", "valuenum_median",
        "most_frequent_value", "most_frequent_comment",
        "sample_count"
    ]
...
def query_ollama(model: str, prompt: str) -> Dict[str, Any]:
    response = ollama.generate(
        model=model,
        prompt=prompt,
        format=ClinicalTerminologyInference.model_json_schema(),
